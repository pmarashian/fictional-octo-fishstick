...
import { selectFileFromPRDsFolder } from "./prompts.mjs";
...
- Generate a descriptive branchName in kebab-case format (e.g., "feature/user-authentication", "ralph/clans-mvp", "feature/payment-integration")
- Branch name should be based on the PRD/task content, not timestamps
- Each task must have sequential IDs starting from 1
- Each task must have 3-5 items in success_criteria array
- All required fields must be present (branchName, userStories, and all task fields)
...
- A descriptive branchName field in kebab-case format (e.g., "feature/user-authentication", "ralph/clans-mvp")
- Branch name should be based on the PRD/task content, not timestamps
- All tasks have sequential IDs starting from 1
- All tasks have success_criteria array with 3-5 items
...
If you see generic items like "TypeScript compilation succeeds without errors" or "Code compiles without errors" in success_criteria, REMOVE them. These are handled by the dev agent prompt/skills, not task-specific success criteria. Only keep task-specific, observable, testable criteria that verify the specific functionality being implemented.`;
            ...
            json_schema: {
              ...
              schema: {
                ...
                properties: {
                  branchName: {
                    type: "string",
                    description: "Descriptive git branch name in kebab-case format (e.g., 'feature/user-authentication', 'ralph/clans-mvp'). Should be based on PRD/task content, not timestamps.",
                  },
                  userStories: {
                    type: "array",
      ...
      // === APPROVAL PATH ===
      if (userChoice === "yes") {
        return parsed; // Return full parsed object with branchName and userStories
      }
...
export async function actionCreateTasks(mcpConfigForAgent) {
  ...
  const taskData = await generateTasksWithAgent(
    prdAbsolutePath,
  ...
  // Generate tasks.json file (taskData includes branchName and userStories)
  const tasksJsonPath = await generateTaskFiles(taskData.userStories, taskData.branchName);

  console.log(`\n✓ Tasks created: ${tasksJsonPath}`);
  return { tasks: taskData.userStories, tasksJsonPath };
}

/**
 * Generate Structured Task Breakdown from Description using Agent + Validation
 *
 * Similar to generateTasksWithAgent but accepts a description string instead of PRD file path.
 * This allows quick task generation from bugs/changes descriptions without requiring a full PRD.
 *
 * Two-phase approach:
 * 1. Agent Phase: Uses AI agent to analyze codebase and description, load skills, read reference files
 * 2. Validation Phase: Uses chat completion with JSON schema to ensure valid JSON output
 *
 * Task Breakdown Process:
 * 1. Agent analyzes codebase to understand current state
 * 2. Agent uses description to identify bugs/changes needed
 * 3. Agent output is validated and converted to JSON using chat completion
 * 4. Parse and validate JSON response
 * 5. Present tasks for user approval
 * 6. Allow regeneration or feedback-based refinement
 * 7. Repeat until approved or max iterations reached
 *
 * @param {string} description - User description of bugs/changes needed
 * @param {number} maxIterations - Maximum refinement iterations (default: 5)
 * @param {Object|null} mcpConfig - MCP configuration for skill loading
 * @returns {Promise<Object>} Object with branchName and userStories array
 * @throws {Error} If max iterations reached without approval or API errors
 */
async function generateTasksFromDescription(
  description,
  maxIterations = 5,
  mcpConfig = null,
) {
  ...
  while (iterations < maxIterations) {
    try {
      ...
      if (!agentOutput) {
        console.log(
          "\n[Phase 1] Agent analyzing codebase and generating task breakdown...",
        );
...
Based on this description of bugs/changes: ${description}

Generate tasks to fix bugs and implement changes.
...
- Create the \`tasks\` directory if it doesn't exist
- Generate a descriptive branchName in kebab-case format (e.g., "feature/user-authentication", "ralph/clans-mvp", "feature/payment-integration")
- Branch name should be based on the task content, not timestamps
- Each task must have sequential IDs starting from 1
- Each task must have 3-5 items in success_criteria array
- All required fields must be present (branchName, userStories, and all task fields)
- Write valid JSON that can be parsed directly
            ...
            json_schema: {
              ...
              schema: {
                ...
                properties: {
                  branchName: {
                    type: "string",
                    description: "Descriptive git branch name in kebab-case format (e.g., 'feature/user-authentication', 'ralph/clans-mvp'). Should be based on task content, not timestamps.",
                  },
                  userStories: {
                    type: "array",
                    description: "Array of user stories broken down from the description",
                    items: {
                      type: "object",
                      properties: {
                        // === TASK IDENTIFICATION ===
...
/**
 * Action: Create Tasks from Description (Quick Mode)
 *
 * Prompts user for a description of bugs/changes (either text input or file path),
 * then generates tasks using generateTasksFromDescription.
 *
 * @param {Object|null} mcpConfigForAgent - MCP configuration for skill loading
 * @returns {Promise<Object|null>} Object with tasks and tasksJsonPath, or null if cancelled
 */
export async function actionCreateTasksFromDescription(mcpConfigForAgent) {
  console.log("\nQuick Task Generation Mode");
  console.log("Enter a description of bugs/changes to generate tasks from...\n");

  // Ask user how they want to provide the description
  const inputMethod = await prompts({
    type: "select",
    name: "method",
    message: "How would you like to provide the description?",
    choices: [
      { title: "Enter text directly", value: "text" },
      { title: "Paste from file", value: "file" },
    ],
  });

  if (!inputMethod.method) {
    console.log("No input method selected. Returning to menu.");
    return null;
  }

  let description = "";

  if (inputMethod.method === "text") {
    // Get text input directly
    const textResponse = await prompts({
      type: "text",
      name: "description",
      message: "Enter description of bugs/changes:",
      multiline: true,
    });

    if (!textResponse.description) {
      console.log("No description provided. Returning to menu.");
      return null;
    }

    description = textResponse.description;
  } else {
    // Select file from prds folder
    const selectedFile = await selectFileFromPRDsFolder();

    if (!selectedFile) {
      console.log("No file selected. Returning to menu.");
      return null;
    }

    // Read file content from prds directory
    const prdsDir = path.join(process.cwd(), "prds");
    const filePath = path.join(prdsDir, selectedFile);
  ...
  if (!description.trim()) {
    console.log("Description is empty. Returning to menu.");
    return null;
  }

  console.log(`\nDescription:\n${description}\n`);

  // Generate tasks using description-based approach with JSON validation
  const taskData = await generateTasksFromDescription(
    description,
    5,
    mcpConfigForAgent,
  );
