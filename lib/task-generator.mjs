...
Output your task breakdown analysis with all required fields. Remember: descriptions = user stories, acceptance criteria = technical details.`;
...
- Valid JSON structure`;
      ...
      for (const task of parsed.userStories) {
        if (
          ...
          console.error(
            `Task ${task.id || "unknown"} missing valid success_criteria array (must have 3-5 items, found ${task.success_criteria?.length || 0})`,
          );
