...
export async function actionCreateTasksFromDescription(mcpConfigForAgent) {
  ...
  if (inputMethod.method === "text") {
    ...
    if (!fileResponse.filePath) {
      console.log("No file path provided. Returning to menu.");
      return null;
    }

    const filePath = fileResponse.filePath.trim();
    ...
    try {
      ...
      description = await fs.readFile(absolutePath, "utf8");
      console.log(`\nLoaded description from: ${absolutePath}`);
    } catch (error) {
