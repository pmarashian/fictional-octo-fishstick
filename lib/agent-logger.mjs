...
export function logAssistantMessage(event, loggingState, logCallback) {
  if (event.message?.content?.[0]?.text) {
    ...
    // Prevent duplicate completion markers from streaming
    // If the accumulated content already contains a completion marker and
    // this chunk also contains one, skip adding this chunk to avoid duplication
    const accumulatedHasMarker = loggingState.accumulatedAssistant.includes("<ralph>COMPLETE</ralph>");
    const chunkHasMarker = text.includes("<ralph>COMPLETE</ralph>");

    if (accumulatedHasMarker && chunkHasMarker) {
      // Skip adding this chunk to prevent duplicate completion markers
      return;
    }
