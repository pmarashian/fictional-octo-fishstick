    ...
    try {
      ...
      if (isResourceExhaustionError(error)) {
        ...
        console.log(
          format(
            "error",
            `  This indicates the AI agent exceeded resource limits and needs to be restarted.`,
          ),
        );
        ...
        // Mark task with error state
        task.completed = false;
        task.notes = `Resource exhaustion: ${error.message}`;
        task.completed_at = new Date().toISOString();
