 ...
 */
export async function generateTaskFiles(tasks) {
  ...
  const tasksData = {
    ...
    userStories: tasks.map((task) => ({
      id: `US-${String(task.id).padStart(3, '0')}`,  // Standardized ID format (US-001, US-002, etc.)
      title: task.description.split(':')[0] || task.description.substring(0, 60),  // Extract or truncate title
      description: task.description,              // Full task description
      acceptanceCriteria: task.success_criteria,  // Success criteria for completion
