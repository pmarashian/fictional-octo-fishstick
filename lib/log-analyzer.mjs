...
export async function analyzeLogs() {
  ...
  const selection = await prompts({
    ...
    initial: 0, // Select "All logs" by default
  });

  if (!selection.files || selection.files.length === 0) {
    console.log("No files selected. Cancelling analysis.");
    return;
  }

  // Determine which files to analyze
  const filesToAnalyze = selection.files.includes("all")
    ? logFiles
    : selection.files;
  ...
  displayAnalysisResults(allResults);
}
...
function analyzeLogData(logEntries, fileName) {
  ...
  // Extract statistics
  const iterations = logEntries.filter((e) =>
    e.message?.includes("Iteration") && e.message?.includes("complete")
  ).length;

  const toolCalls = logEntries.filter((e) =>
    e.message?.includes("TOOL:") || e.level === "tool_call"
  ).length;
  ...
  if (agentHangs.length > 0) {
    issues.push({
      ...
      details: "Agent execution was terminated due to consecutive tool failures.",
