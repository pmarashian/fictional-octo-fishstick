...
import { logAgentEvent, createLoggingState, logLoopError } from "./agent-logger.mjs";
  ...
  while (iterations < maxIterations) {
    ...
    while (retryAttempt <= MAX_CONNECTION_RETRIES) {
      try {
        ...
        if (retryAttempt > 0) {
          logCallback("✓ Connection restored, continuing...");
        }
        break;
      } catch (error) {
        ...
        if (isRetryableConnectionError(error) && retryAttempt < MAX_CONNECTION_RETRIES) {
          retryAttempt++;
          logCallback(
            `⚠️ Connection lost, retrying... (Attempt ${retryAttempt}/${MAX_CONNECTION_RETRIES})`,
          );
