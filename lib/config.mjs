 ...
 *
 * Configuration for handling connection errors (ECONNRESET, ECONNREFUSED, etc.)
 * during agent streaming operations. These settings control automatic retry
 * behavior when network issues interrupt agent communication.
 *
 * - MAX_CONNECTION_RETRIES: Maximum retry attempts per iteration (default: 3)
 * - RETRY_DELAY_MS: Initial retry delay in milliseconds (default: 1000ms)
...
/**
 * Loop Detection and Recovery Configuration
 *
 * Configuration for detecting and recovering from infinite loops during agent execution.
 * These settings prevent the agent from getting stuck in repeated tool calls or
 * exceeding maximum iteration runtime.
 *
 * - MAX_ITERATION_RUNTIME_MS: Maximum runtime per iteration in milliseconds (default: 5 minutes = 300,000ms)
 * - MAX_LOOP_RETRIES: Maximum retry attempts for loop errors before giving up (default: 3)
 */
export const MAX_ITERATION_RUNTIME_MS = 300000; // 5 minutes
export const MAX_LOOP_RETRIES = 3;

/**
 * Resource Exhaustion Recovery Configuration
 *
 * Configuration for handling resource exhaustion errors, particularly context window limits.
 * These settings control recovery delays and backoff behavior when resource limits are hit.
 *
 * - RESOURCE_EXHAUSTION_BASE_DELAY_MS: Base delay before retrying after resource exhaustion (default: 2 seconds)
 * - RESOURCE_EXHAUSTION_BACKOFF_MULTIPLIER: Exponential backoff multiplier (default: 1.5)
 * - MAX_CONTEXT_SIZE_BYTES: Maximum context size before forcing conversation rotation (default: 60KB)
 */
export const RESOURCE_EXHAUSTION_BASE_DELAY_MS = 2000; // 2 seconds
export const RESOURCE_EXHAUSTION_BACKOFF_MULTIPLIER = 1.5;
export const MAX_CONTEXT_SIZE_BYTES = 60000; // ~60KB context limit
 ...
 * Skills Directory
 *
 * Directory containing Agent Skills learned from project progress.
 * Skills are stored in .cursor/skills/ following the Agent Skills specification.
 * Located in: project/.cursor/skills/
 */
export const SKILLS_DIR = path.join(process.cwd(), ".cursor", "skills");
